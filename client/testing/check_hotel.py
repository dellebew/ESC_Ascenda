...
def check_hotel(driver, action, button, location):
    ...
    try:
        ...
        driver.save_screenshot('./client/testing/screenshots/hotels_{}.png'.format(hotelId))
        ...
        for i, roomCard in enumerate(roomCards):
            ...
            # check for amenities
            amenitiesList = roomCard.find_element(By.CLASS_NAME, "room--amenities")
            assert(roomImages.is_displayed())
        ...
        with open(fileLocation, 'a') as f:
            f.write("Failed Loading Time Hotel: {} @ {}".format(hotelId, time.ctime()))
            f.write('\n')
            f.close()
        print(e)
